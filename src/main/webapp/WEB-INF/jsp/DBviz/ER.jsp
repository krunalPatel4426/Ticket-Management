...
<div class="container">
    ...
    <div class="table-info">
        <h3>📋 Database Tables (${tables.size()} tables)</h3>
...
<script>
        ...
        card.innerHTML = `
                ...
                <ul>
                    ${table.columns.slice(0, 5).map(col -> {
                        ...
                        if (col.includes('(PK)')) {
                            className = 'pk';
                            displayCol = col.replace('(PK)', '<span class="pk">🔑 PK</span>');
                        } else if (col.includes('(FK)')) {
                            className = 'fk';
                            displayCol = col.replace('(FK)', '<span class="fk">🔗 FK</span>');
                        }
                        return `<li>${displayCol}</li>`;
                    }).join('')}
                    ${table.columns.length > 5 ? `<li>... and ${table.columns.length - 5} more</li>` : ''}
                </ul>
            `;
    ...
    function exportDiagram() {
        // Note: This is a placeholder - in a real JSP environment, you'd need server-side support
        alert('To export: Right-click on the diagram and select "Save image as..."');
    }
